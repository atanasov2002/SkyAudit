...
async getTrustedAdvisorRecommendations(account: any): Promise<any[]> {
  ...
  const costChecks = (checksResp.checks || []).filter(c => c.category === 'cost_optimization');
...
async getComputeOptimizerRecommendations(account: any): Promise<any[]> {
  ...
  const recsResp = await optimizerClient.send(new GetEC2InstanceRecommendationsCommand({}));
